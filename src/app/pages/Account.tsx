      ...
      <div className="py-10" style={{ backgroundColor: '#902131' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full flex items-center justify-center" style={{ backgroundColor: '#e6bb0c' }}>
              <span className="text-xl" style={{ color: '#902131', fontWeight: 700 }}>
                {user?.name.charAt(0).toUpperCase()}
              </span>
            </div>
            <div>
              <h1 style={{ fontFamily: 'Lora, serif', color: 'white', fontSize: '1.5rem' }}>{user?.name}</h1>
          ...
          <div className="md:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              <button
                ...
                style={{
                  fontWeight: 500,
                  borderLeft: activeTab === 'profile' ? '3px solid #cc323f' : '3px solid transparent',
                  backgroundColor: activeTab === 'profile' ? '#fdf4f3' : 'transparent',
          ...
          <div className="md:col-span-3">
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 style={{ fontFamily: 'Lora, serif', color: '#0f172a', fontSize: '1.3rem' }}>
                  Thông Tin Cá Nhân
                </h2>
                {!editing ? (
                  <button
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setEditing(false)} className="flex items-center gap-1.5 text-gray-500 text-sm hover:text-gray-700">
                      <X className="w-3.5 h-3.5" /> Hủy
                    </button>
                    <button onClick={handleSave} className="flex items-center gap-1.5 text-sm hover:underline" style={{ color: '#cc323f', fontWeight: 600 }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {[
                  ...
                  <div key={field.key} className={field.key === 'email' ? 'sm:col-span-2' : ''}>
                    <label className="block text-sm text-gray-600 mb-1.5" style={{ fontWeight: 500 }}>{field.label}</label>
                    {editing ? (
                      <input
                        type={field.type}
              ...
              {!editing && (
                <div className="mt-6 pt-6 border-t border-gray-100">
                  <Link to="/account/orders" className="inline-flex items-center gap-2 text-sm hover:underline" style={{ color: '#cc323f' }}>
