        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {userOrders.length > 0 && (
            <p className="text-white/60 text-sm mt-1">
              {userOrders.length} đơn hàng mới · {allOrders.length} tổng cộng
            </p>
          )}
        </div>
      </div>
        ...
        {allOrders.length === 0 ? (
          ...
          <div className="space-y-4">
            {allOrders.map((order, idx) => {
              const isNew = idx < userOrders.length;
              return (
                <Link
                  key={`${order.id}-${idx}`}
                ...
                >
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                    ...
                    <span
                      className={`px-3 py-1.5 rounded-full text-xs ${getStatusColor(order.status)}`}
