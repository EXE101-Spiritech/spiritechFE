...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (!form.phone.trim() || !/^0\d{9}$/.test(form.phone)) e.phone = 'Số điện thoại không hợp lệ (10 số, bắt đầu bằng 0)';
    ...
    <div style={{ fontFamily: 'Be Vietnam Pro, sans-serif' }} className="min-h-screen bg-[#f8fafc] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 style={{ fontFamily: 'Lora, serif', color: '#0f172a', fontSize: 'clamp(1.2rem, 3vw, 1.8rem)' }}>Tạo Tài Khoản</h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
          ...
          <div className="mt-6 pt-6 border-t border-gray-100 text-center">
            <p className="text-gray-500 text-sm">
              Đã có tài khoản?{' '}
              <Link to={`/login${redirect !== '/account' ? `?redirect=${redirect}` : ''}`} className="hover:underline" style={{ color: '#cc323f', fontWeight: 600 }}>
                Đăng nhập
              </Link>
            </p>
          </div>
