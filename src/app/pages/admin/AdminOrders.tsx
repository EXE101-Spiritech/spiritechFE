...
const Badge = ({ status, map }: { status: string; map: typeof PAYMENT_LABELS }) => {
...
export default function AdminOrders() {
  ...
  const updateStatus = (id: string, status: 'pending' | 'paid' | 'completed') => {
    setOrders(os => os.map(o => o.id === id ? { ...o, orderStatus: status } : o));
    if (selectedOrder?.id === id) setSelectedOrder(o => o ? { ...o, orderStatus: status } : null);
  };
  ...
  return (
    <div style={{ fontFamily: 'Be Vietnam Pro, sans-serif' }}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h2 className="text-xl font-semibold text-gray-900" style={{ fontFamily: 'Lora, serif' }}>Đơn hàng</h2>
          <p className="text-sm text-gray-500 mt-0.5">{orders.length} đơn • Doanh thu: <span className="font-medium" style={{ color: '#cc323f' }}>{formatCurrency(totalRevenue)}</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {(['pending', 'paid', 'completed'] as const).map(s => {
          ...
          return (
            <button key={s} onClick={() => setFilterStatus(s === filterStatus as any ? 'all' : s)}
              ...
              <p className="text-2xl font-semibold" style={{ color: conf.color }}>{count}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: '#faf9f8' }} className="border-b border-gray-100">
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium">Mã đơn</th>
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium">Khách hàng</th>
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium hidden sm:table-cell">Ngày đặt</th>
                <th className="text-right px-5 py-3.5 text-gray-500 font-medium">Tổng tiền</th>
                <th className="text-center px-5 py-3.5 text-gray-500 font-medium hidden md:table-cell">Thanh toán</th>
                <th className="text-center px-5 py-3.5 text-gray-500 font-medium">Trạng thái</th>
                <th className="text-center px-5 py-3.5 text-gray-500 font-medium">Chi tiết</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {filtered.map(order => (
                <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-3.5 font-mono text-sm font-medium" style={{ color: '#902131' }}>#{order.id}</td>
                  <td className="px-5 py-3.5 text-gray-800">{order.customer}</td>
                  <td className="px-5 py-3.5 text-gray-500 hidden sm:table-cell">{order.date}</td>
                  <td className="px-5 py-3.5 text-right font-semibold" style={{ color: '#cc323f' }}>
                    ...
                    <div className="relative inline-block">
                      <select
                        ...
                        style={{
                          ...
                          background: ORDER_STATUS_LABELS[order.orderStatus]?.bg,
                        }}
                      >
                        <option value="pending">Chờ xử lý</option>
                        <option value="paid">Đã thanh toán</option>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5 font-mono">#{selectedOrder.id}</p>
              </div>
              <button onClick={() => setSelectedOrder(null)} className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="px-6 py-4 space-y-5">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Thông tin giao hàng</p>
                <p className="text-sm font-medium text-gray-800">{selectedOrder.shipping.name}</p>
                ...
                <p className="text-xs text-gray-400">
                  Vận chuyển: {selectedOrder.shipping.method === 'express' ? '🚀 Hỏa tốc' : '📦 Tiêu chuẩn'}
                </p>
              </div>

              {/* Items */}
              <div>
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-3">Sản phẩm đặt hàng</p>
                <div className="space-y-2">
                  {selectedOrder.items.map((item, i) => (
                    <div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                      <div>
                        <p className="text-sm text-gray-800">{item.name}</p>
                        <p className="text-xs text-gray-400">x{item.qty} × {formatCurrency(item.price)}</p>
                      </div>
                      <p className="text-sm font-semibold" style={{ color: '#cc323f' }}>
                  ...
                  <div className="flex justify-between pt-2">
                    ...
                    <span className="text-base font-bold" style={{ color: '#cc323f' }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 rounded-xl p-3">
                  ...
                  <Badge status={selectedOrder.paymentStatus} map={PAYMENT_LABELS} />
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-500 mb-1">Trạng thái đơn</p>
                  <Badge status={selectedOrder.orderStatus} map={ORDER_STATUS_LABELS} />
                </div>
              </div>

              {/* Update Status */}
              <div>
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Cập nhật trạng thái</p>
                <div className="flex gap-2">
                  {(['pending', 'paid', 'completed'] as const).map(s => (
                    <button
                      ...
                      style={selectedOrder.orderStatus === s ? { background: '#cc323f', color: '#fff', borderColor: '#cc323f' } : { color: '#6b7280' }}
                    >
