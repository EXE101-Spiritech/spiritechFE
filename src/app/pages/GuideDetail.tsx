  ...
  if (!article) {
    return (
      <div style={{ fontFamily: 'Be Vietnam Pro, sans-serif' }} className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center">
        ...
        <Link to="/guide" className="underline" style={{ color: '#cc323f' }}>← Quay lại hướng dẫn</Link>
      </div>
    );
  }

  const relatedCombo = article.relatedComboId ? combos.find(c => c.id === article.relatedComboId) : null;
  const others = guideArticles.filter(a => a.id !== id).slice(0, 3);
  ...
  return (
    <div style={{ fontFamily: 'Be Vietnam Pro, sans-serif' }} className="min-h-screen bg-[#f8fafc]">
      <div className="py-10" style={{ backgroundColor: '#902131' }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="text-white/60 text-sm mb-3">
            ...
            <Link to="/guide" className="hover:text-white">Hướng dẫn</Link>
            <span className="mx-2">/</span>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-8">
          ...
          <div className="p-8">
            <div className="flex flex-wrap gap-3 mb-4">
              <span className="text-xs px-2.5 py-1 rounded-full" style={{ backgroundColor: '#fdf4f3', color: '#cc323f' }}>{article.category}</span>
              <span className="text-gray-400 text-xs flex items-center gap-1"><Clock className="w-3 h-3" />{article.readTime} đọc</span>
              <span className="text-gray-400 text-xs flex items-center gap-1"><Calendar className="w-3 h-3" />{article.date}</span>
            </div>

            <h1 style={{ fontFamily: 'Lora, serif', color: '#0f172a', fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', lineHeight: 1.3 }} className="mb-6">
              {article.title}
            </h1>
            ...
            {relatedCombo && (
              <div className="mt-8 pt-6 border-t border-gray-100">
                <div className="rounded-2xl p-5 border" style={{ background: 'linear-gradient(to right, #fdf4f3, #f8fafc)', borderColor: 'rgba(230,187,12,0.3)' }}>
                  <p className="text-sm mb-2" style={{ color: '#cc323f', fontWeight: 600 }}>🎁 Combo phù hợp cho nghi lễ này:</p>
                  ...
                  <div className="flex gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm mb-0.5" style={{ fontFamily: 'Lora, serif', color: '#0f172a', fontWeight: 600 }}>
                        {relatedCombo.name}
                      </p>
                      <p className="text-sm" style={{ color: '#cc323f', fontWeight: 700 }}>
                        {formatCurrency(relatedCombo.price)}
                      </p>
                      <p className="text-xs text-gray-500 mt-0.5">{relatedCombo.items.length} lễ vật · {relatedCombo.occasion}</p>
                    </div>
                  </div>
        ...
        {/* Related articles */}
        <div>
          <h2 style={{ fontFamily: 'Lora, serif', color: '#0f172a', fontSize: '1.3rem' }} className="mb-5">
            Bài viết liên quan
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {others.map(a => (
              <Link key={a.id} to={`/guide/${a.id}`} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all group">
                <div className="h-32 overflow-hidden">
                  <img src={a.image} alt={a.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <div className="p-3">
                  <p className="text-xs text-gray-400 mb-1">{a.category} · {a.readTime}</p>
                  <h4 className="text-sm line-clamp-2" style={{ fontFamily: 'Lora, serif', color: '#0f172a' }}>{a.title}</h4>
                </div>
              </Link>
            ))}
          </div>
